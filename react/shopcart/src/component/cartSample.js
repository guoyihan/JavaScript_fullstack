...
export default class CartSample extends Component {
    constructor (props) {
        ...
        this.state = {
            text: '', //选择的商品名
            goods: [
                { id: 1, text: '你不知道的js', price: 77},
                { id: 2, text: '你不知道的vue', price: 56},
                { id: 3, text: '你不知道的react', price: 32}
            ],
            cart: [],
            history: []
        }
    ...
    addCount = item => {
        const newCart = [...this.state.cart]
        const idx = newCart.findIndex(c => c.text === item.text)
        newCart.splice(idx, 1, {...item, count: item.count + 1}) //将原来的删除,用新的代替
        this.setState({
            cart: newCart,
            history: [...this.state.history, newCart]
        })
    ...
    addToCart(good){
        ...
        const newCart = [...this.state.cart]
        // 查询商品对应的购物项是否存在
        const idx = newCart.findIndex(e => e.text === good.text)
        // 如果存在idx,则取出
